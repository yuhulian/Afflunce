...
object getAverage {
  def main(args: Array[String]): Unit = {
    ...
    val rsRDD = sc.textFile("/user/ss_deploy/workspace/simon/spendingpower/rsprice/p*").map(_.split("\\|")).map(x=>(x(0),x(1)))
    rsRDD.cache()

    val step1RDD = crmFinalRDD.leftOuterJoin(uniFinalRDD).map(x=>(x._1,x._2._1+"|"+x._2._2.getOrElse("-999|-999|-999")))
    ...
    val finalRDD = step4RDD.leftOuterJoin(rsRDD).map(x=>x._1+"|"+x._2._1+"|"+x._2._2.getOrElse("-999")).coalesce(16).saveAsTextFile(resultDir)
