...
/**
  ...
  */
object SpendingPowerWithoutPCA {
  def main(args: Array[String]): Unit = {
    if (args.length != 4) {
      println("Wrong number of arguments!")
      println("Usage: <$PROVINCE> <$K> <$INPUT_DIR> <$RESULT_DIR>")
      System.exit(1)
    }
    val province = args(0)
    val numClusters = args(1).toInt
    val inputDir = args(2)+"/"+province
    val resultDir = args(3)+"/"+province
...
//    })
    ...
    val runs = 2
    val clusters = KMeans.train(scaledDataRDD, numClusters, numIterations, runs)

    //step3. evaluate the result
    val WSSSE = clusters.computeCost(scaledDataRDD)
    println("Within Set Sum of Squared Errors = " + WSSSE)

    //    //step4. get ids and labels, save them to the file
        val out = clusters.predict(scaledDataRDD)
