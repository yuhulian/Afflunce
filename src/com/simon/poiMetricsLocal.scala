...
//      println("numRoamCity-->"+numRoamCity)
    ...
    val jpRDD = sc.textFile("E:\\【05#运维信息】\\~UE文档\\journey_plus.txt").map(_.split("\\|")).map(x=>
      (x(1),x(17),x(42)+x(43))).filter(x=>(x._3.equals("12") ||x._3.equals("21"))).map(x=>(x._1,x._2)).reduceByKey(_+","+_)
    jpRDD.cache()

    jpRDD.take(5).foreach(println)
    ...
    val commuteRDD = jpRDD.map { x =>
      ...
      var strArr: Array[String] = x._2.split(",")
      for (i <- 0 to (strArr.length - 1)) {
        commuteList.add(strArr(i))
        println(commuteList.get(i))
      }

      //5 features we need to collect
      var totalCommuteDistance = 0.0
//      var avgCommuteDistance = 0

      for (i <- 0 to commuteList.size() - 1) {
        totalCommuteDistance += commuteList.get(i).toDouble
      }
      (user_id,totalCommuteDistance/commuteList.size())
    }
//    commuteRDD.take(5).foreach(println)
    poiMetricsRDD.leftOuterJoin(commuteRDD).map(x=>x._1+"|"+x._2._1+"|"+x._2._2.getOrElse(-999)).take(10).foreach(println)
  }
}
